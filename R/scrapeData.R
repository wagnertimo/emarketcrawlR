...
getIntradayContinuousEPEXSPOT <- function(startDate, endDate, product = "60") {
  ...
  for(i in seq(length(dates_array), 1, -2)) {
    ...
    parsedHtml <- htmlParse(content(postResponse, "text", encoding = "UTF-8"))
    r <- rbind(r, parseICEPEXSPOT(parsedHtml, product))
...
parseICEPEXSPOT <- function(htmlDoc, product) {
  library(logging)
  library(XML)
  ...
  if(product == "60") {
    ...
    start_freq <- seq(1, length(times_list)-1, 7)
    ...
    start_freq <- cseq(2, length(times_list)-1, c(3,4))
    # Set the xpath expression to get the right td elements
    xpath <- "id('content')/div/table/tbody/tr/td[contains(@class, 'toggle_15min_info_closed')]/../td/text()"
  }
  else if(product == "15") {
  ...
  for(i in 2:17) {
    ...
    if(i < 10) {
      ...
      if(i == 8 | i == 9) column <- as.numeric(gsub(",", "", tds_list[seq(i, length(tds_list), 17)])) else column <- as.numeric(tds_list[seq(i, length(tds_list)-1, 17)])
      df1 <- cbind(df1, i = column)
    }
    else {
      # buy and sell vol have comma seperated value fot thousands
      if(i == 16 | i == 17) column <- as.numeric(gsub(",", "", tds_list[seq(i, length(tds_list), 17)])) else column <- as.numeric(tds_list[seq(i, length(tds_list)-1, 17)])
  ...
  r <- rbind(df1, df2)
  colnames(r) <- c("DateTime","Low","High","Last","Weighted_Avg","Idx","ID3","Buy_Vol","Sell_Vol","Index_Base","Index_Peak")
